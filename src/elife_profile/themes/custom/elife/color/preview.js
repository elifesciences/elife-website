...
(function ($) {
  Drupal.color = {
    ...
    callback: function(context, settings, form, farb, height, width) {
	  ...
	  $('#preview #preview-main #preview-page-title, #preview #preview-block h2', form).css('color', $('#palette input[name="palette[base]"]', form).val());
      ...
      $('#preview #preview-header', form).css('border-bottom-color', $('#palette input[name="palette[top]"]', form).val());

      $('#preview #preview-site-name', form).css('color', $('#palette input[name="palette[titleslogan]"]', form).val());
      
      // Menu preview
      $('#preview #preview-main-menu', form).css('background-color', $('#palette input[name="palette[top]"]', form).val());
      $('#preview #preview-main-menu', form).css('border-top-color', $('#palette input[name="palette[base]"]', form).val());
      $('#preview #preview-main-menu-links a', form).css('color', $('#palette input[name="palette[bottom]"]', form).val());
      $('#preview #preview-main-menu-links li:first-child a', form).css('color', $('#palette input[name="palette[titleslogan]"]', form).val());
    }
  };
})(jQuery);
